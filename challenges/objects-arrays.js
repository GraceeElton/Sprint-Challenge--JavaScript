...
function getSchools (schools){
  ...
  for (let i = 0; i < schools.length; i++) {
    newArray.push(schools[i].university);
  }
  return newArray.sort();
}

console.log(getSchools(graduates));
...
function displayNames(info){
  const displayNames = [];
    info.forEach(item => {
      displayNames.push(`Name: ${item.animal_name}, Scienfific: ${item.scientific_name}.`)
    })
    return displayNames;
}
